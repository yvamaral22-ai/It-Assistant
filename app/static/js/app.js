...
if (root) {
  const sessionId = sessionStorage.getItem('it-session-id'); let count = 0; const history = [];
  ...
  const render = (node, nodeId) => {
    ...
    if (node.type === 'question') {
      ...
      card.querySelectorAll('.option').forEach(button => button.onclick = async () => {
        try { const result = await api(`/api/sessions/${sessionId}/answer`, {method:'POST',body:JSON.stringify({node_id:nodeId,value:button.dataset.value})}); history.push({node,nodeId}); render(result.node,result.node_id); } catch(exc){error.textContent=exc.message;}
      });
      ...
      card.querySelectorAll('.finish').forEach(button => button.onclick = () => finish(button.dataset.status));
    ...
    } else { finish('unresolved'); }
    document.querySelector('#back-button').hidden = history.length === 0;
  };
  const finish = async status => {
    try { const result = await api(`/api/sessions/${sessionId}/finish`, {method:'POST',body:JSON.stringify({status})});
      if(status==='not_tested'){card.insertAdjacentHTML('beforeend','<p>Sem problema. Volte quando tiver testado a orientação.</p>');return;}
  ...
  document.querySelector('#back-button').onclick=()=>{const previous=history.pop();if(previous)render(previous.node,previous.nodeId)};
  ...
  if(!sessionId){card.innerHTML='<p class="error">Sessão não encontrada. Volte ao início.</p>';} else api(`/api/sessions/${sessionId}`).then(r=>render(r.node,r.session.current_node_id)).catch(exc=>error.textContent=exc.message);
}
